...
namespace Decatron.Default.Controllers
{
    ...
    public class GameCommandController : ControllerBase
    {
        ...
        {
            // Intentar obtener el channelOwnerId de los claims, si no existe usar el userId actual
            ...
            return GetUserId(); // Si no hay channelOwnerId, es su propio canal
        }
        ...
        public async Task<IActionResult> GetCurrentCategory()
        {
            try
            {
                ...
                {
                    return Forbid("No tienes permisos para acceder a esta función");
                }
        ...
        public async Task<IActionResult> UpdateCategory([FromBody] UpdateCategoryRequest request)
        {
            try
            {
                ...
                if (!await _permissionService.CanAccessAsync(userId, channelOwnerId, "game"))
                {
                    return Forbid("No tienes permisos para cambiar la categoría");
                }
                ...
                var success = await GameUtils.UpdateCategoryAsync(_configuration, userInfo.TwitchId, request.Category, userInfo.AccessToken);

                if (success)
                {
                    await GameUtils.SaveCategoryToHistoryAsync(_configuration, userInfo.Login, request.Category, userInfo.Login);
                    _logger.LogInformation($"Categoría actualizada por {userInfo.Login}: {request.Category}");
        ...
        public async Task<IActionResult> ToggleGameCommand([FromBody] ToggleGameCommandRequest request)
        {
            try
            {
                ...
                {
                    return Forbid("Solo los usuarios con control total pueden activar/desactivar comandos");
                }
