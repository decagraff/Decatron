...
namespace Decatron.Scripting.Core
{
    public class ScriptParser
    {
        ...
        private SendStatement ParseSendStatement(string[] tokens, int lineNumber)
        {
            ...
            Console.WriteLine($"[PARSER] ParseSendStatement - tokens: [{string.Join(", ", messageTokens)}]");

            var expression = ParseExpression(messageTokens, lineNumber);
            Console.WriteLine($"[PARSER] ParseSendStatement - expression type: {expression.GetType().Name}");
        ...
        private Expression ParseSingleToken(string token, int lineNumber)
        {
            ...
            if (token.StartsWith("\"") && token.EndsWith("\""))
            {
                ...
                Console.WriteLine($"[PARSER] Parseando string literal: '{content}'");
        ...
        {
            Console.WriteLine($"[TOKENIZER] Input: '{line}'");
            ...
            Console.WriteLine($"[TOKENIZER] Output: [{string.Join(", ", tokens)}]");
