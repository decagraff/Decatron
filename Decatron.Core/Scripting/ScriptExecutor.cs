...
namespace Decatron.Scripting.Core
{
    public class ScriptExecutor
    {
        ...
        private async Task ExecuteStatementSequence(List<ScriptNode> statements, ScriptExecutionContext context, ScriptExecutionResult result)
        {
            for (int i = 0; i < statements.Count; i++)
            {
                ...
                if (statement is WhenStatement whenStmt && HasComparisonOperator(whenStmt.Condition))
                {
                    ...
                    {
                        Console.WriteLine($"[EXECUTOR] Detectada cascada de {whenSequence.Count} when statements");
        ...
        private async Task ExecuteWhenCascade(List<WhenStatement> whenStatements, ScriptExecutionContext context, ScriptExecutionResult result)
        {
            ...
            {
                Console.WriteLine($"[EXECUTOR] Evaluando condición en cascada...");
                ...
                Console.WriteLine($"[EXECUTOR] Condición resultado: {conditionIsTrue}");

                if (conditionIsTrue)
                {
                    Console.WriteLine($"[EXECUTOR] Ejecutando bloque then de la cascada y saliendo");
            ...
            Console.WriteLine($"[EXECUTOR] Ninguna condición en cascada fue verdadera");
        }
        ...
        {
            Console.WriteLine($"[EXECUTOR] Ejecutando SetStatement: variable='{setStmt.VariableName}'");
            Console.WriteLine($"[EXECUTOR] Expresión a evaluar: {setStmt.Value?.GetType().Name}");

            var value = await EvaluateExpression(setStmt.Value, context);
            Console.WriteLine($"[EXECUTOR] Valor evaluado: '{value}'");

            var variableType = DetermineVariableType(value);
            Console.WriteLine($"[EXECUTOR] Tipo determinado: {variableType}");

            context.SetVariable(setStmt.VariableName, value, variableType);
            Console.WriteLine($"[EXECUTOR] Variable '{setStmt.VariableName}' guardada con valor '{value}'");
        }
        ...
        private async Task<object> EvaluateExpression(Expression expr, ScriptExecutionContext context)
        {
            switch (expr)
            {
                ...
                case VariableExpression variable:
                    Console.WriteLine($"[EXECUTOR] Buscando variable: '{variable.VariableName}'");
                    Console.WriteLine($"[EXECUTOR] Variables disponibles: {string.Join(", ", context.Variables.Keys)}");
