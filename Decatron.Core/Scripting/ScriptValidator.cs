...
namespace Decatron.Scripting.Core
{
    public class ScriptValidator
    {
        ...
        {
            Console.WriteLine($"[VALIDATOR] Iniciando validación de script: {script.Length} caracteres");

            if (string.IsNullOrWhiteSpace(script))
                throw new ScriptParseException("El script no puede estar vacío", 0, "");

            var lines = script.Split('\n');
            Console.WriteLine($"[VALIDATOR] Script dividido en {lines.Length} líneas");

            for (int i = 0; i < lines.Length; i++)
            {
                var line = lines[i].Trim();
                Console.WriteLine($"[VALIDATOR] Procesando línea {i + 1}: '{line}'");

                if (string.IsNullOrEmpty(line)) continue;

                ValidateLine(line, i + 1);
                Console.WriteLine($"[VALIDATOR] Línea {i + 1} validada correctamente");
            }

            Console.WriteLine("[VALIDATOR] Validación completa exitosa");
        }
