...
namespace Decatron.Custom.Commands
{
    public class CreateCommand : ICommand
    {
            ...
            ScriptingService scriptingService)
        {
            ...
            CounterFunction.Initialize(configuration);
        }
        ...
        public async Task ExecuteAsync(string username, string channel, string message, IMessageSender messageSender)
        {
            ...
            Console.WriteLine($"Attempting to create command: {commandName} with response: {response}, restriction: {restriction}, and active status: {isActive}");
            ...
            try
            {
                ...
                Console.WriteLine($"Channel Name: {channel}, Broadcaster ID: {broadcasterId}");
        ...
        {
            var scriptKeywords = new[] { "set ", "when ", "then", "else", "end", "send ", "roll(", "pick(", "count(" };
        ...
        private async Task CreateScriptedCommand(string commandName, string response, string channelName, long broadcasterId, string restriction, bool isActive, string createdBy, IMessageSender messageSender)
        {
            ...
            {
                Console.WriteLine($"Error creating scripted command: {ex.Message}");
        ...
        private async Task CreateNormalCommand(string commandName, string response, string channelName, string broadcasterId, string restriction, bool isActive, string createdBy, IMessageSender messageSender)
        {
            ...
            {
                Console.WriteLine($"Error creating normal command: {ex.Message}");
        ...
        private async Task<bool> CommandExists(string commandName, string channelName)
        {
            ...
            {
                Console.WriteLine($"Error checking if command exists: {ex.Message}");
                return false;
            }
        }
    }
